...
with st.sidebar:
    ...
    st.divider()
    st.markdown(
        "<small style='color:#444'>RAG SQL Agent v1.0 · Agentra Lab<br>"
        "Embeddings: sentence-transformers · DB: DuckDB · Store: ChromaDB</small>",
        unsafe_allow_html=True
    )
...
api_key = SessionState.get(SessionState.API_KEY)
files_loaded = SessionState.get(SessionState.FILES_LOADED)
...
if submitted and user_input.strip():
    ...
    # Cap chat history at 50 messages to prevent memory bloat
    chat_history = SessionState.get(SessionState.CHAT_HISTORY)
    if len(chat_history) > 50:
        SessionState.set(SessionState.CHAT_HISTORY, chat_history[-50:])
